...
const Canvas = ({ toolMenuRef }) => {
  ...
  const activeCampaignId = useSelector(
    (state) => state.sessionManager.activeCampaignId
  );
  ...
  const dragStopHandler = (event, data) => {
    if (activeViewPos) {
      if (activeViewPos.x !== data.x || activeViewPos.y !== data.y)
  ...
  const cardDropHandler = (event) => {
    ...
    if (cardCollection[targetCardId]) {
      if (!cardCollection[targetCardId].views[activeViewId]) {
        ...
        let xCalculation =
          Math.round((event.clientX - GRID.size - GRID.size) / GRID.size) *
          GRID.size;
        if (xCalculation < 0) xCalculation = 0;
        let yCalculation =
          Math.round((event.clientY - GRID.size - GRID.size) / GRID.size) *
          GRID.size;
  ...
  if (cardCollection) {
    for (let cardId in cardCollection) {
      if (
        cardCollection[cardId].views &&
        cardCollection[cardId].views[activeViewId]
      ) {
        cardList = [
          ...cardList,
          <Card
    ...
    >
      {userId && !activeCampaignId ? (
        <div className='empty-screen'>
          No active project loaded. Please select your project or create a new
          one.
        </div>
      ) : activeViewId ? (
        <div className='scale-view' style={scaleViewStyle}>
          <Rnd
            // position
